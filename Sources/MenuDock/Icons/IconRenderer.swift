...
enum IconRenderer {
    ...
    /// Renders `draw` into 1× and 2× bitmaps and packs both into one `NSImage`, so the status
    /// item picks the right one per display without AppKit having to interpolate.
    private static func compose(size: NSSize, scales: [CGFloat] = [1, 2], draw: () -> Void) -> NSImage {
